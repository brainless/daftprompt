//! Scripted stand-in for `codex-acp`'s stdio wire behavior, used by
//! `daftprompt-acp`'s own scripted tests so they are repeatable without live
//! Codex credentials or network access, per Epic 014 Task 1's acceptance
//! criterion "Scripted adapter tests run without network access or Codex
//! credentials."
//!
//! Adapted from the Epic 014 Task 0 compatibility spike's harness
//! (`scratch/acp-spike/src/bin/fake_adapter.rs`; see
//! `epics/research/014-acp-adapter-compatibility.md` for how these traffic
//! shapes were captured). Response bodies for `initialize`, `session/new`,
//! `session/prompt`'s `session/update` stream, and the final prompt
//! response are copied (`fixtures/*`) from a real
//! `node --import tsx src/index.ts` run against the local
//! `~/Projects/codex-acp` clone (git revision `97d260e`, package version
//! `1.4.0`) on 2026-08-17. The permission-request fixture is instead
//! reconstructed from `CodexApprovalHandler.ts` because no live run produced
//! one on that workstation (see the fixture file and the findings doc for
    ...
    while let Some(line) = rx.recv().await {
        ...
        match method {
            ...
            "session/prompt" => {
                ...
                if prompt_text.contains("TRIGGER_UNKNOWN_UPDATE") {
                    // Exercise Design Decision #3: unknown session/update
                    // variants, unrecognized notification methods, and
                    // unrecognized reverse-request methods must all be
                    // preserved as generic diagnostics, not crash the
                    // session. None of these are real ACP wire shapes.
                    write_line(
                        &mut stdout,
                        &json!({
                            "jsonrpc": "2.0",
